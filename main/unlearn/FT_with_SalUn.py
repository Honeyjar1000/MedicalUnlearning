...
def ForgetClass(dataset: RetinaMNIST, 
    ...
    forget_imgs = np.array([])
    forget_labels = np.array([])
...
def FT_Forget_SalUn(model, TrainData:RetinaMNIST, ValData:RetinaMNIST, TestData:RetinaMNIST, criterion, optimizer, mask, device, 
              class_to_forget,
              batch_size,
              epochs): 
    
    TrainData_F = ForgetClass(TrainData, class_to_forget)
    ValData_F = ForgetClass(ValData, class_to_forget)

    ForgetTrainLoader, ForgetValLoader, ForgetTestLoader = get_custom_loaders(batch_size, TrainData_F, ValData_F, TestData)
